...
import { 
  ...
  BarChart4, PieChart, Flame, FileText, X
} from 'lucide-react';
...
const DashboardPage = () => {
  ...
  useEffect(() => {
    setLoading(true);
    // Simulation structurelle détaillée alignée sur le DRF amélioré
    setTimeout(() => {
  ...
  // FONCTION : GÉNÉRER PDF DE LA MAIRIE
  const exportPDF = async () => {
    const input = pdfRef.current;
    if (!input) return;
    
    // Pour ne pas imprimer les boutons etc.
  ...
  // ----- COMPOSANTES MODALES (POP-UPS CLIQUABLES) ----- //
  const ModalAlerts = () => (
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {/* Affichage conditionnel des Modales au premier plan */}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-10" ref={pdfRef}>
        ...
        {/* --- MAIRIE --- */}
        {!loading && role === 'MUNICIPAL_AGENT' && stats?.metrics?.health_score_pct && (
          <section className="animate-fade-in-up bg-white p-6 rounded-2xl shadow-sm border border-gray-200/60">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
              ...
              {/* Le bouton déclenche la modale "Alerte Active" */}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="rounded-2xl border border-gray-200/60 overflow-hidden h-[300px] relative">
                ...
                <div className="absolute top-2 right-2 bg-black/60 text-white text-[10px] p-2 rounded backdrop-blur">
                  🔴 Alertes / 🟢 Fréquentation (Visites)
                </div>
              </div>

              {/* Top Fréquentation (Liste) */}
